...
import static dprs.controller.WriteController.DYNAMO_WRITE;
...
public class DataManager {
        ...
        InMemoryDatabase.INSTANCE.entrySet().stream().forEach((entrySet) -> {
            ...
            if(isThisSyncCoordinator(key)){
                ...
                final List<NodeAddress> destinationAdressesForKey = chord
                        ...
                        .stream()
                        .filter(nodeAddress -> {
                            return nodeAddress.getHash().compareTo(chord.getSelfIndexInChord())
                                    != 0;
                        })
    ...
    private boolean isThisSyncCoordinator(String key) {
        ...
        final Integer destinationAdressesForKey = chord.findDestinationAdressesForKey
                (key, 1).get(0).getHash();
